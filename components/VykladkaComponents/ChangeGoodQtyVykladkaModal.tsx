...
import {GoodsRow} from '../../functions/PocketProvfreeGoods';
import {PocketProvfreeGoodsSet} from '../../functions/PocketProvfreeGoodsSet';

const ChangeGoodQtyVykladkaModal = observer(
  (
    ...
    const changeQty = async () => {
      if (newQty.replaceAll(' ', '').length === 0) props.setmodalVisible(false);
        ...
        try {
          setloading(true);
          const response = await PocketProvfreeGoodsSet({
          ...
          if (mounted) {
            ...
            console.log('Ошибка ChangeGoodQtyVykladkaModal отловлена в бэк');
        }
      }
    };
      ...
      <ModalTemplate {...props}>
        <View
          ...
          <WhiteCardForInfo>
            ...
            />
            <TitleAndDiscribe
              title={'Количество в магазине'}
              discribe={goodInfo?.ShopQty}
            />
          </WhiteCardForInfo>
          <View style={{height: 8}} />
          <WhiteCardForInfo
...
export default ChangeGoodQtyVykladkaModal;
