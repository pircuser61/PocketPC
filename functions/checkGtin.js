  ...
  ,'\x1b[0m');
      ...
      soapRequest
        ...
        .then(response => {
          ...
          let responseXML=response['SOAP-ENV:Envelope']['SOAP-ENV:Body'][0]["ws_ora_gateResponse"][0]["xmlOut"][0]
          parseString(responseXML,(err,responseXMLJS)=>{
            if(responseXMLJS['ServicesResult']["OutParameters"][0]["ServiceResult"][0]['goods'][0]==='')
            {
              fail('Неизвестный товар')
            }
          if(responseXMLJS["ServicesResult"]["OutParameters"][0]["ServiceResult"][0]["goods"][0])
          {
            if(responseXMLJS["ServicesResult"]["OutParameters"][0]["ServiceResult"][0]["goods"][0]["good"][0]["gtin"].length)
            {
              let gtins=[]
                responseXMLJS["ServicesResult"]["OutParameters"][0]["ServiceResult"][0]["goods"][0]["good"][0]["gtin"].map(r=>{
                  //console.log(JSON.stringify(r));
                  gtins=[...gtins,r]
                })
                if(gtins.length)
                {
                console.log('\x1b[32m','Функция checkGtin отработала нормально\n<----------------------------------------------------------------------->' ,'\x1b[0m');
